...
const typeBadge: Record<FollowUpType, { bg: string; text: string; label: string }> = {
  ...
  referral: { bg: "bg-green-100", text: "text-green-700", label: "Referral" },
  medication: { bg: "bg-orange-100", text: "text-orange-700", label: "Medication" },
  appointment: { bg: "bg-teal-100", text: "text-teal-700", label: "Appointment" },
...
export const FollowUpCard: React.FC<FollowUpCardProps> = ({
      ...
      <div className="rounded-lg border border-indigo-200 bg-indigo-50/30 p-3 space-y-2">
        <div>
          ...
          <input
            ...
            className="w-full rounded border border-slate-300 px-2 py-1 text-sm focus:border-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400"
          />
        </div>
  ...
  return (
    <div className="group flex items-start gap-3 rounded-lg border border-slate-200 bg-white p-3 transition-colors hover:border-slate-300">
      {/* Checkbox placeholder */}
      <div className="mt-0.5 h-4 w-4 flex-shrink-0 rounded border-2 border-slate-300" />
