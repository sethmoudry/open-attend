    ...
    <div className="card">
      ...
      {!collapsed && (
        <div className="mt-3 space-y-2">
          {orders.length === 0 ? (
            <p className="text-xs text-slate-400 dark:text-slate-500">No orders detected yet.</p>
          ) : (
            orders.map((order, idx) => (
              <div
                key={`${order.name}-${order.cpt_code}-${idx}`}
                className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-2.5 text-xs"
              >
                <div className="flex items-center justify-between">
                  ...
                  {order.cpt_code && (
                    <span className="rounded bg-slate-100 dark:bg-slate-900 px-1.5 py-0.5 font-mono text-[10px] text-slate-500 dark:text-slate-400">
                      CPT {order.cpt_code}
                    </span>
                  )}
                </div>
                {order.indication && (
                  <p className="mt-1 text-slate-500 dark:text-slate-400">{order.indication}</p>
                )}
              </div>
            ))
          )}
