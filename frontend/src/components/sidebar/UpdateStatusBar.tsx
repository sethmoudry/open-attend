...
export default function UpdateStatusBar({ sessionId, onUpdated }: UpdateStatusBarProps) {
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2">
      <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-slate-400">
        {status ? (
          <>
            <span title="Alerts">
              <span className="font-medium text-slate-400 dark:text-slate-500">Alerts</span>{" "}
              {formatTime(status.alerts.seconds_until_due)}
            </span>
            <span className="text-slate-300 dark:text-slate-600">|</span>
            <span title="Medications">
              <span className="font-medium text-slate-400 dark:text-slate-500">Meds</span>{" "}
              {formatTime(status.medications.seconds_until_due)}
            </span>
            <span className="text-slate-300 dark:text-slate-600">|</span>
            <span title="Differential">
              <span className="font-medium text-slate-400 dark:text-slate-500">Diff</span>{" "}
      ...
      <button
        ...
        disabled={refreshing}
        className="flex items-center gap-1 rounded-md bg-clinical-50 dark:bg-clinical-900/30 px-2 py-1 text-[11px] font-medium text-clinical-700 dark:text-clinical-300 transition-colors hover:bg-clinical-100 dark:hover:bg-clinical-900/50 disabled:opacity-50"
