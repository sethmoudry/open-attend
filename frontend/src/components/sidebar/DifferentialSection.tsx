          ...
          {differential.length === 0 ? (
            ...
            <ol className="space-y-1.5">
              {differential.map((dx, idx) => (
                <li
                  key={`${dx}-${idx}`}
                  className="flex items-baseline gap-2 rounded-md px-2 py-1.5 text-xs transition-all hover:bg-slate-50"
                >
                  <span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-clinical-100 text-[10px] font-bold text-clinical-700">
                    {idx + 1}
                  </span>
