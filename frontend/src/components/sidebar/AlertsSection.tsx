...
const priorityStyles: Record<AlertPriority, string> = {
  ...
  info: "border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300",
};
    ...
    <div className="card">
      ...
      >
        <h3 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
          ...
          Alerts
          {alerts.length > 0 && (
            <span className={`ml-1 inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full px-1.5 text-[10px] font-bold ${urgentCount > 0 ? "bg-alert-500 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300"}`}>
              {alerts.length}
            </span>
          )}
        </h3>
