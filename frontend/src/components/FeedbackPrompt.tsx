...
export default function FeedbackPrompt({ sessionId }: FeedbackPromptProps) {
  ...
  if (submitted) {
    return (
      <div className="rounded-lg border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 p-5 text-center">
        <svg
        ...
        <p className="mt-1 text-xs text-emerald-600 dark:text-emerald-400">
          Your input helps us improve Open Attend.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5">
      ...
      <textarea
        ...
        rows={3}
        className="mb-4 w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 placeholder:text-slate-400 dark:text-slate-500 focus:border-clinical-400 focus:outline-none focus:ring-1 focus:ring-clinical-400"
      />
      ...
      <button
        ...
        className={`w-full rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${
          rating === 0
            ? "cursor-not-allowed bg-slate-100 dark:bg-slate-900 text-slate-400 dark:text-slate-500"
