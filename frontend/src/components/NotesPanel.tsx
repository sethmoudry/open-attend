    ...
    <div className="space-y-4 px-6 py-4">
      ...
      <section>
        ...
        <p className="text-sm text-slate-700 dark:text-slate-300">
          {chiefComplaint || (
            <span className="italic text-slate-400 dark:text-slate-500">Awaiting data...</span>
          )}
        </p>
      </section>
      ...
      <section>
        ...
        {medications.length === 0 ? (
          <p className="text-sm italic text-slate-400 dark:text-slate-500">None yet</p>
        ) : (
            ...
            {medications.map((med, i) => (
              <li key={i} className="flex items-start gap-1.5 text-sm text-slate-700 dark:text-slate-300">
                ...
                <span>
                  ...
                  {med.dose && <span className="text-slate-500 dark:text-slate-400"> {med.dose}</span>}
                  {med.frequency && (
                    <span className="text-slate-500 dark:text-slate-400"> &middot; {med.frequency}</span>
                  )}
                </span>
              </li>
            ))}
      ...
      <section>
        ...
        {alerts.length === 0 ? (
          <p className="text-sm italic text-slate-400 dark:text-slate-500">No alerts</p>
        ) : (
          <div className="space-y-1.5">
            {alerts.map((alert, i) => (
              <div
                key={i}
                className="flex items-start gap-2 rounded-md border border-red-200 dark:border-red-700 bg-red-50 dark:bg-red-900/30 px-3 py-2"
              >
      ...
      <section>
        ...
        {differential.length === 0 ? (
          <p className="text-sm italic text-slate-400 dark:text-slate-500">Awaiting data...</p>
        ) : (
          <ol className="list-inside list-decimal space-y-0.5 text-sm text-slate-700 dark:text-slate-300">
