...
export default function TranscriptReference({
  ...
  const speakerBg: Record<string, string> = {
    doctor: "bg-blue-50",
    physician: "bg-blue-50",
    patient: "bg-emerald-50",
    unknown: "bg-slate-50",
    other: "bg-slate-50",
  };
      ...
      {!collapsed && (
        <div className="border-t border-slate-100">
          ...
          <div className="px-6 py-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2 pl-10 pr-4 text-sm text-slate-800 placeholder-slate-400 focus:border-clinical-300 focus:outline-none focus:ring-1 focus:ring-clinical-300"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
          ...
          {showPinMenu && selectedText && (
            <div className="mx-6 mb-2 flex items-center gap-2 rounded-lg border border-clinical-200 bg-clinical-50 px-3 py-2">
              ...
              {SECTION_OPTIONS.map((s) => (
                <button
                  ...
                  className="rounded bg-white px-2 py-1 text-xs font-medium text-slate-700 shadow-sm transition-colors hover:bg-clinical-100 hover:text-clinical-800"
                >
                  {s.abbrev} - {s.label}
                </button>
              ))}
                ...
                <div
                  ...
                  className={`cursor-pointer rounded-lg px-3 py-2 transition-colors ${
                    highlightedId === chunk.id
                      ? "border border-clinical-300 bg-clinical-50"
                      : `${speakerBg[chunk.speaker] || "bg-slate-50"} border border-transparent hover:border-slate-200`
                  }`}
                >
                  <div className="mb-1 flex items-center gap-2">
                    <span
                      className={`text-xs font-semibold capitalize ${speakerColor[chunk.speaker] || "text-slate-400"}`}
                    >
                      {chunk.speaker}
                    </span>
                  ...
                  <p className="select-text text-sm leading-relaxed text-slate-700">
                    {search ? (
