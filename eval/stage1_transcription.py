...
async def run_stage1():
    ...
    counter = {"done": 0}
    remaining = [e for e in manifest if e["id"] not in completed]
    total = len(manifest)

    async def _process_entry(client: httpx.AsyncClient, i: int, entry: dict):
        async with semaphore:
            logger.info("[Stage 1] [%d/%d] Processing %s (%s)...", i + 1, total, entry["id"], entry.get("specialty", "?"))
            ...
            async with csv_lock:
                ...
                csv_file.flush()
                counter["done"] += 1
                logger.info("[Stage 1] Completed %d/%d", counter["done"], len(remaining))

    async with httpx.AsyncClient(timeout=httpx.Timeout(ASR_TIMEOUT, connect=30.0)) as client:
        tasks = [_process_entry(client, i, entry) for i, entry in enumerate(remaining)]
