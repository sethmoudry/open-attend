...
from llm import call_medgemma_json, set_active_session
from models import (
    AlertPriority,
    AlertType,
    ClinicalAlert,
...
from agents import CHUNK_ANALYSIS_PROMPT_TEMPLATE, ORCHESTRATOR_SYSTEM_PROMPT
...
SOAP_REFRESH_INTERVAL = 1  # refresh SOAP draft every N chunks
...
def _normalize_alert_key(msg: str) -> str:
    """Normalize alert message for fuzzy dedup — lowercase, strip punctuation, collapse whitespace."""
    import re
    return re.sub(r'\s+', ' ', re.sub(r'[^\w\s]', '', msg.lower())).strip()


def _dedup_alerts(existing: list, new_alerts: list) -> list:
    """Merge new alerts into existing, deduplicating by normalized message content."""
    existing_keys = {_normalize_alert_key(a.message if hasattr(a, "message") else str(a)) for a in existing}
    for alert in new_alerts:
        msg = alert.message if hasattr(alert, "message") else str(alert)
        key = _normalize_alert_key(msg)
        # Also check if any existing key contains this key or vice versa (substring dedup)
        is_dup = key in existing_keys or any(key in ek or ek in key for ek in existing_keys if len(ek) > 20)
        if not is_dup:
...
def _context_from_session(session: Session) -> ClinicalContext:
    ...
    ctx = ClinicalContext(
        ...
        allergies=list(session.allergies),  # seed from FHIR pre-loaded allergies
        family_history=[],
        exam_findings=[],
        orders=[],
...
async def process_chunk(
    ...
    Sets the active session for per-session LLM usage tracking.
    ...
    # D4+D5: Consolidated alert detection — throttled (30s Fibonacci-increasing)
    if _alert_throttle.is_due():
        all_session_meds_for_alerts = [
            Medication(name=m.get("name", ""), dose=m.get("dose"), frequency=m.get("frequency"))
        ...
        tasks["alerts"] = asyncio.create_task(
            generate_alerts(
                ...
                medications=all_session_meds_for_alerts,
    ...
    if new_symptoms and _diff_throttle.is_due():
        ...
        # Merge family history with active conditions from FHIR for richer context
        diff_history = ctx.family_history + [
            f"Active condition: {c}" for c in session.conditions
        ]
        tasks["differential"] = asyncio.create_task(
            build_differential(all_symptoms, diff_history)
        )
    ...
    # Consolidated alerts from generate_alerts tool
    new_alerts.extend(results.get("alerts", []))
    ...
    # Drug interaction alerts — only moderate+ severity (skip low/informational)
    for flag in interaction_flags:
        if flag.severity.value in ("low", "informational"):
            continue
        new_alerts.append(
            ClinicalAlert(
